...
def build_rfm(trx: pd.DataFrame) -> pd.DataFrame:
    ...
    reference_date = pd.Timestamp(cfg.DATE_MAX) + pd.Timedelta(days=1)
    total_days = (pd.Timestamp(cfg.DATE_MAX) - pd.Timestamp(cfg.DATE_MIN)).days + 1

    # 1. Hitung Kuantitas Harian untuk CV2
    # Karena tidak ada kolom qty, asumsikan 1 baris transaksi = 1 unit
    ...
    # 2. Hitung ADI dan CV2
    adi_cv2_stats["ADI"] = total_days / adi_cv2_stats["active_days"]
    # Jika std NaN (transaksi hanya terjadi di 1 hari), jadikan 0
    adi_cv2_stats["qty_std"] = adi_cv2_stats["qty_std"].fillna(0)
    adi_cv2_stats["CV2"] = (adi_cv2_stats["qty_std"] / adi_cv2_stats["qty_mean"]) ** 2
